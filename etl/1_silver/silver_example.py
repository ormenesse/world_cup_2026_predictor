"""Example silver ETL job — tutorial template.
...
def process_data(self, input_tables):
    ...
    input_tables : dict[str, polars.DataFrame]
        **THIS IS NOT YOUR JOB'S INPUT FILES.** It is a dict of
        ...
        silver values must match another job's `output_table_name` prefixed by
    ...
    -------
    polars.DataFrame
        Persisted by ETLBase to `silver_<output_table_name>` (partitioned
