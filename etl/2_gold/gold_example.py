"""Example gold ETL job — tutorial template.
...
def process_data(self, input_tables):
    ...
    input_tables : dict[str, polars.DataFrame]
        **THIS IS NOT YOUR JOB'S INPUT FILES.** It is a dict of
        ...
        gold values must match another job's `output_table_name` prefixed by
    ...
    -------
    polars.DataFrame
        Persisted by ETLBase to `gold_<output_table_name>` (partitioned
